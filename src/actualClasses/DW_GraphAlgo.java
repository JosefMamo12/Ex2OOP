...
public class DW_GraphAlgo implements dw_graph_algorithms {
	...
	public directed_weighted_graph copy() {
		...
		while(itr.hasNext()) {
			...
			node_data newCopiedNode = new NodeData ();
			newCopiedNode.setInfo(current.getInfo());
			newCopiedNode.setTag(current.getKey());
			newCopiedNode.setWeight(current.getWeight());
			newCopiedNode.setLocation(current.getLocation());
	...
	private void dijkestra (int src) {
		node_data first = this._graph.getNode(src);
		...
		for (int i = 0; i < _graph.nodeSize(); i++) {
			...
			current.setTag(0);
			ArrayList <edge_data> nodeNei = new ArrayList<edge_data>(_graph.getE(current.getKey())); 
			for (edge_data neibo : nodeNei) {
				if(neibo!=null) {
					double neiboWei = neibo.getWeight();
					node_data neiboNode =  _graph.getNode(neibo.getDest());
	...
	private node_data mindistance() {
		...
		Iterator <node_data> itr = _graph.getV().iterator();
		while(itr.hasNext()) {
			node_data minNode =  itr.next();
			if(minNode.getWeight() < minValue)
				minValue = minNode.getWeight();
			key = minNode.getKey();
